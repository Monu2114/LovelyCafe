...
const textures: MaterialTexture[] = [
  { 
    ...
    image: "https://images.unsplash.com/photo-1542384557-0824d90731ee?q=80&w=2574&auto=format&fit=crop"
  },
  { 
    id: "paper", 
    name: "Aged Pages", 
    ...
    image: "https://images.unsplash.com/photo-1586075010923-2dd4570fb338?q=80&w=2574&auto=format&fit=crop"
  },
  { 
    id: "wood", 
    name: "Oak Grain", 
  ...
  return (
    <section className="py-32 px-4 md:px-12 bg-[#F8F5EF]">
      <div className="mb-16 text-center">
          <span className="text-xs font-sans uppercase tracking-widest text-[#C98A82]">The Elements</span>
          <h2 className="text-5xl font-serif text-[#3A2F2F] mt-4">Material Library</h2>
      </div>
      ...
      <AnimatePresence>
        {selectedId && (
          ...
          >
            {textures.map(item => item.id === selectedId && (
              <motion.div 
                layoutId={`card-${item.id}`}
                key={item.id}
              ...
              >
                ...
                <div className="p-12 flex flex-col justify-center">
                    <span className="text-[#C98A82] text-xs font-sans uppercase tracking-widest mb-4">Texture Detail</span>
                    <h3 className="font-serif text-4xl text-[#3A2F2F] mb-6">{item.name}</h3>
                    <p className="font-serif text-lg text-[#3A2F2F]/80 italic mb-8">"{item.description}"</p>
                    
                    <div className="border-t border-[#D9D6D2] pt-8">
                        <p className="font-sans text-sm text-[#7C8B6A] leading-relaxed">
                            {item.detail}. At The Lovely Café, every surface tells a story. 
                            We believe touch is the first language of comfort.
                        </p>
                    </div>
                </div>
              </motion.div>
